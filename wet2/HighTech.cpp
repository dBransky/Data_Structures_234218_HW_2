...
void HighTech::RemoveEmployee(int EmployeeId) {
    ...
    if (employee->IsSalaryBiggerThanZero()) {
        ...
        Company *company = companies.GetCompany(correctCompany);
        company->IncreaseAmountOfEmployees(-1);
        companies.GetCompany(employee->GetCompanyId())->GetCompanyEmployees().remove(
...
void HighTech::EmployeeSalaryIncrease(int EmployeeId, int SalaryIncrease) {
    ...
    companies.GetCompanyById(companyId)->GetCompanyEmployees().insert(SalaryId(employee->GetSalary(), EmployeeId),
                                                                      employee);
}
...
void HighTech::AverageBumpGradeBetweenSalaryByGroup(int CompanyId, int lowerSalary, int higherSalary,
    ...
    if (CompanyId > 0) {
        totalAmount += companies.GetCompany(CompanyId)->GetCompanyEmployees().AmountMinMax(
                SalaryId(higherSalary, INT32_MAX), SalaryId(lowerSalary, 0));
        totalSum += companies.GetCompany(CompanyId)->GetCompanyEmployees().SumMinMax(SalaryId(higherSalary, INT32_MAX),
